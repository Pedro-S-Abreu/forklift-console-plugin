...
test.describe('Plan Details - Migration Type', { tag: '@downstream' }, () => {
  ...
  test('should edit migration type', async ({
    ...
    await test.step('Open edit modal and verify radio buttons', async () => {
      ...
      await expect(planDetailsPage.detailsTab.warmMigrationRadio).toBeChecked();
      await expect(planDetailsPage.detailsTab.coldMigrationRadio).not.toBeChecked();
    });
    ...
    await test.step('Select cold migration and save', async () => {
      ...
      await expect(planDetailsPage.detailsTab.coldMigrationRadio).toBeChecked();
